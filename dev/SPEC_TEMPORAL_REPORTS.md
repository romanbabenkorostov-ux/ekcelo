...
[3] Фотоотчёт по проекту (3 фото/A4, EXIF, заголовки по структуре)
[4] Формат output (MD / DOCX / оба) — текущий: оба
[Q] Выход

Выбор: _
```

Конвертация-как-отдельный-пункт (бывший пункт 3 v1 spec'а) удалена:
output-формат выбирается через пункт 4 один раз на сессию (см. §9.1
dual-rendering). Если пользователь выбрал «DOCX-only» или «оба» —
DOCX-файлы создаются прямо при генерации отчёта в пункте 1/2/3.
...
### §8.5 Подменю 3: Фотоотчёт по проекту

**Источник фотографий:** папка `<project>/images/` (стандартное место для
project-фото; см. EXIF UserComment схема). Иерархические заголовки — по
структуре подпапок (как в `06_photo_report_to_docx_v3.py`); подписи —
с привязкой к КН/BU/документу через EXIF UserComment payload (поле
`doc_id`, §4.4).

**Output:**
- DOCX: `<project>/reports/report_photo_<ts>.docx` — DOCX-native через
  `report_builder.DocxNativeBuilder` (заимствует логику `06` напрямую —
  3 фото/A4, SEQ-нумерация, TOC, EXIF-подписи, COM-обновление).
- MD (опционально, если выбрано «оба»): `report_photo_<ts>.md` —
  с relative-ссылками на JPG (`![caption](../images/file.jpg)`); без
  SEQ-нумерации (markdown её не поддерживает).

### §8.6 Подменю 4: Формат output

Toggle между `md` / `docx` / `both`. Сохраняется на время сессии.
...
Перед выходом — если выбран `both` и какой-то рендерер упал — повторно
предлагается генерация (`Y/n`).

---

## §9. DOCX Emission — Dual-rendering через `parser/utils/report_builder.py`

### §9.0 Revision rationale (2026-05-24)

Изначально (§9 v1) предполагалась цепочка `MD → DOCX` через
конвертер. После изучения готового скрипта `parser/scripts/pirushin_sosn_rocha_06_photo_report_to_docx_v3.py`
(см. §17) — отказываемся от MD-промежуточного формата для DOCX-output.

**Причина:** MD→DOCX конвертация всегда теряет качество (SEQ-поля для
автонумерации «Рисунок N» / TOC-поля для оглавления / стилизованные
подписи / EXIF-метаданные в подписях не выживают через MD-формат).
`06_photo_report_to_docx_v3.py` показал, что DOCX-native генерация через
python-docx + `OxmlElement` для полей даёт результат сразу пригодный
для Word без F9 (через COM-обновление).

### §9.1 Dual emission strategy

Один абстрактный builder `ReportBuilder` (в `parser/utils/report_builder.py`),
два конкретных рендерера:
...
class ReportBuilder(Protocol):
    ...
    def figure(self, image_path: Path, caption: str) -> None: ...   # авто-SEQ
    def footnote_ref(self, source_id: str, desc: str) -> str: ...    # вернёт «[^N]» или «¹»
    def sources_block(self) -> None: ...                              # развёртывает накопленный SourceTracker
    def save(self, out_path: Path) -> None: ...

class MarkdownBuilder(ReportBuilder): ...        # → .md  (для git-friendly review)
class DocxNativeBuilder(ReportBuilder): ...      # → .docx (DOCX-native, без MD-промежуточного)
```

Пользователь в подменю выбирает один из режимов (или оба):
- **MD-only** — для review, git-friendly;
- **DOCX-only** — для отправки руководству, со всеми SEQ/TOC-полями;
- **MD + DOCX** — оба файла одной сессии, идентичный контент.

### §9.2 Что заимствуется из `06_photo_report_to_docx_v3.py`

См. подробно §17. Кратко:

- **SEQ-поля** для автонумерации «Рисунок N» / «Таблица N»: `_insert_seq_field()`
  (06:320-352).
- **TOC-поле** в начале документа: `add_table_of_contents()` (06:429-479).
- **COM-обновление полей** через MS Word `Word.Application` → `Fields.Update()`
  + `TablesOfContents.Update()`: `update_fields_via_word()` (06:575-646).
- **EXIF-извлечение** для подписей к фотодокументам: `extract_exif()`
  (06:108-157), `_gps_to_decimal()` (06:93-105), `normalize_for_docx()`
  (06:163-181) — Word не всегда учитывает EXIF-ориентацию, поэтому
  ротация применяется заранее.
- **Иерархические заголовки H2..H9** по структуре (для отчётов — по
  объектам / бенефициарам / периодам, не по папкам).
- **Игнор-список папок** для подписей при включении фото-секции в отчёт.
- **3 фото на страницу A4** с подгонкой по слотам + натуральный размер
  для маленьких — для опционального фоторазмещения в отчёте.

### §9.3 Fallback цепочка (только если `python-docx` недоступен)

```
1. python-docx (offline, pure-Python) — primary.
   На Linux/CI: только python-docx; SEQ/TOC видны при открытии в Word.
   COM-обновление пропускается (не Windows).

2. LibreOffice (`soffice --headless --convert-to docx`) — если рендерим
   MD-промежуточный (например при отказе python-docx по ImportError) и
   хотим DOCX как side-output.

3. MS Word COM (Win-only, через pywin32) — для пост-обработки готового
   DOCX (обновление полей TOC/SEQ без ручного F9). Не блокирующий шаг:
   если COM недоступен — файл валиден, поля обновятся при первом
   F9 в Word.
```

`06_photo_report_to_docx_v3.py` уже реализует пункт 3 (COM-обновление) —
переиспользуем его `update_fields_via_word()` как util.

### §9.4 API

```python
def make_report(
    target_format: Literal["md", "docx", "both"],
    out_dir: Path,
    timestamp: str,
    report_kind: Literal["osv_recon", "pledges", "photo"],
) -> list[Path]:
    """
    Возвращает список путей к созданным файлам.
    timestamp одинаков для всех файлов одной CLI-сессии (§8.7).
    """
```

### §9.5 Тест-план

- CI (Linux + python-docx): оба рендерера, без COM-обновления. Проверка
  что DOCX открывается через `docx.Document(path)` без warnings и
  содержит ожидаемые SEQ/TOC-узлы через `oxml.etree` enumeration.
- Manual smoke-test на Win10: README со шагами проверки COM-обновления
  (запуск `09_v1` → создаётся DOCX → открывается в Word → SEQ/TOC
  отрисованы без F9).
...
7. **Viewer-team integration**: при ratify viewer'ом схемы EXIF v1.1
   (новое поле `doc_id`) — нужно ли расширить protocol pre-selection
   (`postMessage ekcelo.graph.select`) дополнительным параметром
   `as_of_date` для синхронизации с timeline-UI? Сейчас contract 2.11.0
   протокол по single-point-in-time. См. CORRESPONDENCE/013 §4 FYI.
...
| PR | Что | Зависимости |
|---|---|---|
| **PR-α** *(этот)* | spec в `dev/SPEC_TEMPORAL_REPORTS.md` + пост 013 + §9 контракта (informative bullet); reference-копия `06_photo_report_to_docx_v3.py` в `parser/scripts/` | — |
...
| **PR-γ** | `parser/utils/report_builder.py` (`ReportBuilder` Protocol + `MarkdownBuilder` + `DocxNativeBuilder`, заимствуя SEQ/TOC/EXIF/COM-update из `06`); `SourceTracker` util; интеграция с `resolve_state` и `founder_chain_has_pledge` | β |
| **PR-δ** | `09_make_reports_v1.py` (пункт 2 меню — залоги; ОСВ — `--unimplemented`) поверх `report_builder` | γ |
| **PR-ε** | `09_v1` пункт 1 (ОСВ-сверка) поверх `report_builder` | δ |
| **PR-ζ** | `09_v1` пункт 3 (фотоотчёт) — извлечение photo-pipeline из `06_v3` в `report_builder.figure()` + интеграция с EXIF UserComment payload (`doc_id`) | δ |
| **PR-η** | state-tags v2 — расширенный namespace handling + 1 ingester (например, парсер юр.состояния из ОСВ-комментариев) | ε |

Владелец сам решает приоритет и порядок PR-β..η; spec — отправная точка.
...
| Photo→DOCX pipeline (3/A4, SEQ, TOC, EXIF, COM-update) | `parser/scripts/pirushin_sosn_rocha_06_photo_report_to_docx_v3.py` | Базовый референс для `report_builder.DocxNativeBuilder` (§9, §17); photo-pipeline для §8.5 (фотоотчёт) — PR-γ/ζ |
...
## §17. Заимствования из `06_photo_report_to_docx_v3.py`

### §17.1 Контекст

Готовый скрипт `parser/scripts/pirushin_sosn_rocha_06_photo_report_to_docx_v3.py`
(копия в репо для reference; не запускается в pipeline) — production-ready
DOCX-генератор фотоотчётов из дерева папок. Покрывает паттерны, которые
надо переиспользовать в `report_builder` и в подменю 3 фотоотчёта.

Эта секция — каталог заимствований с указанием line-ref'ов и контекста
использования в `report_builder`.

### §17.2 SEQ-поля для автонумерации

**Из `06`:** `_insert_seq_field()` (строки 320-352) — вставка
`<w:fldChar>` / `<w:instrText> SEQ <name> \* ARABIC </w:instrText>` /
`<w:fldChar>` через `OxmlElement`. Cached placeholder отрисовывается до
обновления полей.

**Использование в `report_builder`:**
- `DocxNativeBuilder.figure(image_path, caption)` → SEQ `Рисунок`.
- `DocxNativeBuilder.table(headers, rows, title)` → SEQ `Таблица`
  для названия таблицы.

### §17.3 TOC-поле в начале документа

**Из `06`:** `add_table_of_contents()` (строки 429-479) — добавляет
заголовок «Оглавление» (Heading 1, чтобы он сам не попал в TOC) + поле
`TOC \o "2-9" \h \z \u`. `<w:dirty>true</w:dirty>` — Word поймёт, что
надо обновить при открытии (или COM-обновление сделает это
автоматически).

**Использование:** `DocxNativeBuilder.__init__` создаёт TOC автоматически
если документ имеет ≥ 2 заголовков H2.

### §17.4 COM-обновление полей через MS Word

**Из `06`:** `update_fields_via_word()` (строки 575-646) — открывает
DOCX через `win32com.client.DispatchEx('Word.Application')`,
`Fields.Update()` + `TablesOfContents.Update()` × 2 прохода (второй
устаканивает номера страниц), `Save()` + `Close()` + `Quit()` +
`CoUninitialize()`. Graceful-degradation: если pywin32 не установлен
или Word недоступен — warning в stdout, возвращает `False`; файл
валиден, поля обновятся при F9.

**Использование:** `DocxNativeBuilder.save()` пытается вызвать
`update_fields_via_word()` как post-step (Win10-only); на Linux/CI —
просто skip.

### §17.5 EXIF-извлечение

**Из `06`:**
- `extract_exif()` (строки 108-157) — DateTimeOriginal + DateTime +
  GPS (lat/lon) + orientation.
- `_gps_to_decimal()` (строки 93-105) — конвертация
  `(degrees, minutes, seconds, ref)` → decimal degrees.
- `normalize_for_docx()` (строки 163-181) — применяет
  EXIF-ориентацию через `ImageOps.exif_transpose` если orientation ∈
  {3, 6, 8}; сохраняет JPEG quality=92 в temp-папку. Word не всегда
  учитывает EXIF-ориентацию — поэтому ротация делается заранее.

**Использование в `report_builder`:**
- `DocxNativeBuilder.figure(image_path, caption)` — вызывает
  `extract_exif()` для подписи (дата съёмки + GPS) и
  `normalize_for_docx()` перед `add_picture()`.
- **Дополнительно для temporal-отчётов:** читать EXIF UserComment
  payload (схема `docs/EXIF_USERCOMMENT_SCHEMA.md` v1.1) — поле
  `doc_id` для ссылки на `documents.json`, поле `cad` для группировки
  фото по объектам.

### §17.6 Иерархические заголовки H2..H9

**Из `06`:**
- `collect_sections()` (строки 494-518) — DFS-обход с накоплением
  `(chain_of_headings, list_of_images)`. Игнор-список папок
  `IGNORE_FOLDERS = {'фото', 'фотографии', 'фотография', 'foto'}`
  работает только для подписей (структура обходится полностью).
- `_format_folder_name()` (строки 278-284) — `_` → пробел, первая
  буква заглавной.
- `emit_sections()` (строки 521-572) — рендерит только **новые**
  заголовки (diff с `state.emitted`), вставляет page-break перед
  новой секцией если уже что-то размещали.
- `add_heading_safe()` (строки 414-418) — clamp `level` к [1, 9].

**Использование в `report_builder`:**
- Для отчётов §8.3 (ОСВ) и §8.4 (залоги) — иерархия идёт по семантике
  отчёта (счёт → объект; залогодержатель → объект), не по папкам.
  Алгоритм diff заголовков переиспользуется через
  `MarkdownBuilder.heading()` и `DocxNativeBuilder.heading()`.
- Для §8.5 (фотоотчёт) — иерархия по подпапкам `<project>/images/`,
  игнор-список расширяется на parser-специфичные имена
  (`Не_распределено`, `_data`, `_raw` etc.).

### §17.7 Подгонка 3 фото на страницу A4

**Из `06`:**
- Константы (строки 72-87): `PAGE_MARGIN_CM = 2.0`, `CONTENT_W/H_CM`,
  `PHOTOS_PER_PAGE = 3`, `CAPTION_RESERVE_CM = 1.5`,
  `HEADING_RESERVE_CM = 1.0`, `SLOT_W/H_CM`, `ASSUMED_DPI = 96.0`.
- `computed_size_cm()` (строки 184-206) — финальный размер с подгонкой
  по слоту, **без увеличения** маленьких фото (натуральный размер @
  96 dpi). Учитывает EXIF-ориентацию (swap w↔h если orient ∈ {5,6,7,8}).
- `setup_document()` (строки 254-275) — A4 + поля 2 см + стиль Normal
  (Arial 10pt, без space_before/after).

**Использование:** `DocxNativeBuilder.figure()` принимает опциональный
`layout="grid_3_per_page"` (для фотоотчёта); по умолчанию — full-width.

### §17.8 Подписи к рисункам

**Из `06`:** `add_image_caption()` (строки 354-380), `build_caption_text()`
(строки 287-317) — italic 9pt серый цвет, формат:
`Рисунок N. <Папка1, Папка2, …,> файл <имя> (съемка от <дата>,
координаты ~ <lat>, <lon>).`

**Использование в `report_builder` для temporal-отчётов:**
Формат подписи изменяется на:
`Рисунок N. <object_descriptor>, файл <имя> [^N]`
где `[^N]` — footnote-ссылка на `doc_id` через `SourceTracker`.

### §17.9 Жёсткие разрывы страниц

**Из `06`:** `insert_page_break()` (строки 421-426) — `add_paragraph()
.add_run().add_break(WD_BREAK.PAGE)`.

**Использование:** между секциями отчёта (новый счёт ОСВ, новый
залогодержатель) и между объектами в фотоотчёте.

### §17.10 Console UI patterns

**Из `06`:**
- `class C` (строки 50-56) — ANSI цвета (GREEN/RED/YELLOW/CYAN/BOLD/RESET).
- `cp()` (строки 59-60) — print с цветом.
- `ask_yn()` (строки 652-662), `ask_dir()` (строки 665-684) — интерактивные
  prompt'ы с retry и поддержкой ru/en.
- `main()` (строки 687-766) — три шага: ШАГ 1 ввод → подсчёт +
  подтверждение → ШАГ 2 сборка → ШАГ 3 сохранение + COM-обновление.
- KeyboardInterrupt / Exception handler (строки 770-779).

**Использование в `09_v1`:** прямое заимствование utility-функций
(`ask_yn`, `ask_dir`, `cp`, `C`) в отдельный `parser/utils/cli_io.py`
или inline в `09_v1` (зависит от того, насколько другие скрипты будут
их использовать — решение в PR-γ).

### §17.11 Что НЕ заимствуется

- **`IGNORE_FOLDERS = {'фото', 'фотографии', ...}`** — узкоспецифично
  для фотоотчёта; в temporal-отчётах не применимо. Для §8.5
  (фотоотчёт) — может быть расширено parser-специфичными именами.
- **A4-разметка** для ОСВ-сверки и залоговой таблицы — отчёт обычно
  не table-of-photos, а table-of-rows. Используется default page-setup
  python-docx (Letter US или A4 без custom-полей; решение в PR-γ).
- **Hardcoded heading-text** «Фотофиксация при идентификации объектов
  {name}» (строка 553 `06`) — это template только для фотоотчётов;
  temporal-отчёты используют свои templates.
