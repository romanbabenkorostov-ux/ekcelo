...
#
# КН в РФ имеет формат XX:XX:XXXXXXXX:XX[/N], где /N — необязательный
# суффикс «часть» (например 90:25:020102:119/1 — часть земельного участка).
# В файловой системе слэш запрещён, поэтому используется маска с `__`:
# 90_25_020102_119__1. Контракт зафиксирован в md 052 §2.7a.
INN_UL_RE  = re.compile(r"(?<!\d)(\d{10})(?!\d)")           # 10 цифр — ЮЛ
INN_FL_RE  = re.compile(r"(?<!\d)(\d{12})(?!\d)")           # 12 цифр — ФЛ/ИП
CN_RE      = re.compile(r"(\d{2}:\d{2}:\d{1,8}:\d{1,8}(?:/\d+)?)")
CN_UND_RE  = re.compile(r"(?<!\d)(\d{2})_(\d{2})_(\d{1,8})_(\d{1,8})(?:__(\d+))?(?!\d)")
...
def find_cads_in_text(text: str) -> list[str]:
    """Извлекает КН в канонической форме '61:44:0050706:31' (или с '/N')
    из текста. Поддерживает три написания:
      • с двоеточиями и опц. '/N'        (61:44:0050706:31[/1])
      • с подчёркиваниями и опц. '__N'   (61_44_0050706_31[__1])
      • «плоское» 12-15 цифр подряд      (61440040713308)
    """
    if not text: return []
    out: list[str] = []
    ...
    for m in CN_UND_RE.finditer(text):
        ...
        if m.group(5):  # хвост __N — часть КН
            cn += f"/{m.group(5)}"
        out.append(cn)
...
def load_objects_index(root: Path) -> tuple[dict[str, str], dict[str, set[str]]]:
    ...
    for f in objdir.glob("*.json"):
        ...
        # `data.cadastral_number` — приоритет; fallback — имя файла-маски
        # (паспорт сохраняется как `json/objects/<маска>.json`).
        cn = data.get("cadastral_number") or _mask_to_cn(f.stem) or f.stem
...
def _cn_to_mask(cn: str) -> str:
    """Канонический КН → маска для файловой системы.

    Преобразование: `:` → `_`, `/` → `__`.
    Пример: '90:25:020102:119/1' → '90_25_020102_119__1'.

    Используется во всех именах файлов и папок, где встречается КН.
    Контракт зафиксирован в md 052 §2.7a (`CN_MASK_RE`).
    """
    return cn.replace(":", "_").replace("/", "__")


def _mask_to_cn(mask: str) -> str | None:
    """Маска КН → канонический КН.

    Обратное преобразование к `_cn_to_mask`. Извлекает первый подходящий
    КН из строки (имени файла/папки). Возвращает None, если маска не
    распознана.
    """
    m = CN_UND_RE.search(mask)
    if not m:
        return None
    cn = f"{m.group(1)}:{m.group(2)}:{m.group(3)}:{m.group(4)}"
    if m.group(5):
...
# Сохранён короткий alias `cad_to_token` для обратной совместимости с
# вызовами в старых сайдкарах / индексах. Внутри 07 используется только
# `_cn_to_mask` — единая точка правды для конвенции имён.
cad_to_token = _cn_to_mask
...
def _write_object_passport(root: Path, cn: str, rec: dict,
    ...
    f = objdir / f"{_cn_to_mask(cn)}.json"
...
def _route_non_photo(root: Path, src: Path) -> Path | None:
    ...
    if sfx in (".txt", ".md"):
        return root / TXT_MD_DIR / src.name
    if sfx in (".sig", ".asc", ".p7s"):
        # ЭП-подписи переезжают рядом с основным файлом — упростим:
...
# Папки-«контейнеры»: не несут адресной/КН-информации, при поиске
# ближайшей «адресной» родительской папки их пропускаем.
_CONTAINER_FOLDER_NAMES = {
    "документы", "документация", "docs",
    "фото", "foto", "photos", "images",
    "output", "input", "src", "tmp", "temp", "архив",
    "коммерческие аналоги", "аналоги", "оригинал",
    "новая папка", "new folder", "scan", "сканы",
}

def _is_container_folder(name: str) -> bool:
    if not name: return True
    n = name.strip().lower()
    if n in _CONTAINER_FOLDER_NAMES: return True
    # любая папка с семантической меткой (Фото/Кухня/Состояние внутри…)
    return folder_semantic(name) is not None


def _load_egrn_dump(json_path: Path) -> tuple[dict[str, str], dict[str, set[str]]]:
    """Читает egrn_*.json (tables-схема парсера) → ({cn: object_type},
    {cn: address_tokens}). Возвращает пустые dict, если формат не тот."""
    ...
    types: dict[str, str] = {}
    addrs: dict[str, set[str]] = {}
    for tbl in ("building_objects", "land_objects"):
        for row in tables.get(tbl, []) or []:
            cn = row.get("cad_number"); addr = row.get("address")
            ot = row.get("object_type")
            if not ot and tbl == "land_objects": ot = "land"
            if cn and ot:   types[cn] = ot
            if cn and addr: addrs[cn] = address_tokens(addr)
    return types, addrs


def build_dump_index(src: Path, root: Path) -> dict:
    """Один проход по дампу: собираем
       - addr2cn:  {абс.путь адресной папки: cn}  — связь по имени
         подпапки, содержащей КН в названии;
       - obj_types/obj_addrs: {cn: type}, {cn: address_tokens} —
         объединение json/objects/<cn>.json (если есть в болванке) и
         всех egrn_*.json, найденных в самом дампе."""
    types, addrs = load_objects_index(root)

    # Сканируем egrn_*.json внутри дампа (пользователь принёс свежую базу)
    for jp in src.rglob("egrn_*.json"):
        t2, a2 = _load_egrn_dump(jp)
        for k, v in t2.items(): types.setdefault(k, v)
        for k, v in a2.items(): addrs.setdefault(k, v)

    # Адресные папки → cn: ищем подпапки, в названии которых есть КН,
    # привязываем к ближайшей НЕ-контейнерной родительской папке.
    # Заодно вытаскиваем из имени подпапки подсказку типа объекта
    # («61440040713308_квартира_Суворова_52_кв_7» → room).
    addr2cn: dict[str, str] = {}
    for sub in src.rglob("*"):
        if not sub.is_dir(): continue
        cads = find_cads_in_text(sub.name)
        if not cads: continue
        cn = cads[0]
        ot_hint = detect_category_from_text(sub.name)
        if ot_hint: types.setdefault(cn, ot_hint)
        # Поднимаемся вверх до src, ищем первую НЕ-контейнерную папку
        for parent in sub.parents:
            if parent == src or parent == src.parent: break
            if _is_container_folder(parent.name): continue
            addr2cn.setdefault(str(parent), cn); break

    return {"addr2cn": addr2cn, "types": types, "addrs": addrs}


def _find_cn_for_photo(jpg: Path, src: Path, idx: dict, evidence: list[str]
                       ) -> str | None:
    """Поиск КН для одного JPG по приоритету (см. docstring scan_unsorted)."""
    parts = list(jpg.relative_to(src).parts)
    full_text = " ".join(parts)

    # 1) КН в полном пути
    cads = find_cads_in_text(full_text)
    if cads:
        evidence.append(f"cn_in_path:{cads[0]}"); return cads[0]

    # 2) КН в братских PDF/XML в любой родительской папке (вверх до src)
    visited_dirs: set[Path] = set()
    for parent in [jpg.parent, *jpg.parent.parents]:
        if parent in visited_dirs: continue
        visited_dirs.add(parent)
        if parent == src.parent: break
        try: entries = list(parent.iterdir())
        except Exception: continue
        for sibling in entries:
            if sibling == jpg or not sibling.is_file(): continue
            sfx = sibling.suffix.lower()
            if sfx not in (".pdf", ".xml"): continue
            # сначала пытаемся найти КН в имени файла — быстро
            cads = find_cads_in_text(sibling.name)
            if not cads and sfx == ".pdf":
                cads = find_cads_in_text(extract_text_any(sibling) or "")
            elif not cads and sfx == ".xml":
                try: cads = find_cads_in_text(
                    sibling.read_text(encoding="utf-8", errors="ignore"))
                except Exception: cads = []
            if cads:
                evidence.append(f"cn_in_sibling:{sibling.name}"); return cads[0]
        if parent == src: break

    # 3) Адресная папка из индекса дампа (КН в подпапке)
    for parent in [jpg.parent, *jpg.parent.parents]:
        if parent == src.parent: break
        cn = idx["addr2cn"].get(str(parent))
        if cn:
            evidence.append(f"addr_folder:{parent.name}→{cn}"); return cn
        if parent == src: break

    # 4) Адресный матчинг любой родительской папки (по egrn_*.json /
    #    json/objects/) — ≥ 2 общих токенов адреса
    obj_addrs = idx["addrs"]
    if obj_addrs:
        for parent in [jpg.parent, *jpg.parent.parents]:
            if parent == src.parent or parent == src: break
            if _is_container_folder(parent.name): continue
            tokens = address_tokens(parent.name.replace("_", " ").replace("-", " "))
            cands = [(c, len(tokens & ta)) for c, ta in obj_addrs.items()
                     if len(tokens & ta) >= 2]
            cands.sort(key=lambda x: -x[1])
            if len(cands) == 1:
                evidence.append(f"address_unique:{parent.name}")
                return cands[0][0]
            if len(cands) > 1:
                evidence.append(f"address_ambiguous:{parent.name}:"
                                + ",".join(c for c, _ in cands[:5]))
                return None  # неоднозначность — фото оставляем
    return None
...
def scan_unsorted(root: Path) -> dict:
    ...
    Не-фото файлы (PDF/XML/DOC/XLS/DB/HTML/JSON/KML/KMZ/TXT/MD/SIG)
    маршрутизируются по расширению в соответствующие корневые папки
    (`_route_non_photo`).

    Для фото (*.jpg, *.jpeg) ищется КН по приоритету:
      1) КН в цепочке родительских папок (включая «плоский» 12-15-цифр);
      2) КН в братском PDF/XML — поиск в каждой родительской папке
         от папки фото вверх до Не_распределено/;
      3) адресная папка из индекса дампа (`build_dump_index.addr2cn`):
         подпапка с КН в названии связывает родительскую адресную папку
         со своим КН — например `БСадовая_111/Документы/61440040713343…/`
         делает «БСадовая_111» адресной папкой КН 61:44:0040713:343;
      4) адресный матчинг любой родительской НЕ-контейнерной папки по
         egrn_*.json (внутри дампа) или json/objects/ (≥ 2 общих токена).

    Тип объекта берётся из egrn_*.json/json/objects/ по полю object_type
    (для land_objects дефолт = land); при отсутствии — из текста пути
    (квартира→room, жилой дом→building, ЗУ/земучасток→land и т.п.).

    Семантические подпапки нормализуются по словарю и сохраняются в
    полном виде иерархии."""
    ...
    for f in sorted(src.rglob("*")):
        ...
        # Не-фото — маршрутизация по расширению
        if not _is_photo(f):
            target = _route_non_photo(root, f)
            if target is None:
                plan["items"].append({"from": rel_from, "to": None,
                    "kind": kind_label, "reason": "неизвестный тип файла"})
            else:
                plan["items"].append({"from": rel_from,
                    "to": str(target.relative_to(root)).replace("\\", "/"),
                    "kind": kind_label, "evidence": ["routing_by_extension"]})
            continue

        # Фото: ищем КН
        evidence: list[str] = []
        cn = _find_cn_for_photo(f, src, idx, evidence)

        # Категория: сначала из индекса, потом из текста пути
        parts = list(f.relative_to(src).parts)
        full_text = " ".join(parts)
        category = None
        if cn:
            ot = idx["types"].get(cn, "")
            category = OBJ_TYPE_TO_CAT.get(ot.lower() if isinstance(ot, str) else "")
            if not category:
        ...
        if cn and category:
            ...
            plan["items"].append({"from": rel_from,
                "to": str(target.relative_to(root)).replace("\\", "/"),
                "kind": "photo", "cn": cn, "category": category,
                "semantic": sem_path, "evidence": evidence})
        else:
            plan["items"].append({"from": rel_from, "to": None,
                "kind": "photo",
                "reason": ("адресный конфликт" if any("address_ambiguous"
                            in e for e in evidence)
                           else "КН/категория не определены"),
                "evidence": evidence})

    # Сводим конфликты адресов в отдельный блок для удобства просмотра
    for it in plan["items"]:
        for e in it.get("evidence") or []:
            if e.startswith("address_ambiguous:"):
                _, folder, cands = e.split(":", 2)
                rec = plan["address_ambiguous"].setdefault(folder, {
                    "candidates": cands.split(","), "photos": 0})
                rec["photos"] += 1
