...
⚠️ ИЗВЕСТНАЯ ПРОБЛЕМА (сообщено пользователем 2026-08-20): парсер
перестал работать — не парсит НСПД. Причина не диагностирована в этой
сессии (нет доступа к nspd.gov.ru, чтобы посмотреть, что изменилось на
сайте) — предположительно, НСПД изменил разметку/логику страницы карты
(структура m-* веб-компонентов/Shadow DOM, WFS/PKK endpoints, URL-схема
`nspd.gov.ru/map?query=...` — любое из этого могло поменяться без
предупреждения, как уже бывало между версиями этого файла, см. историю
"Что нового" ниже). TODO на будущее: поискать актуальные обходные пути
на форумах/GitHub (сообщество вокруг Rosreestr/НСПД API достаточно
активное — CADASTRE.RU, форумы кадастровых инженеров, issues в открытых
парсерах ЕГРН/НСПД на GitHub), доработать под новую разметку. До
доработки — считать все пять источников контура (`network_capture`,
`wfs`, `pkk`, `ol_state`, `screenshot_cv`) непроверенными.
...
async def parse_one(page, context, capture, cadastral_number, session, depth=0):
    ...
    # Слои "Здания" и "ОКС" на карте — не для WFS/PKK (те идут по layer ID
    # NSPD_OKS_IDS напрямую, от чекбоксов не зависят), а для OL-state/CV
    # fallback (extract_contour): те читают/скриншотят то, что РЕАЛЬНО
    # отрисовано на карте, и если слой визуально выключен — объект для них
    # не существует. Напоминание печатается на каждом объекте, а не один
    # раз за сессию: goto() — полная навигация, состояние чекбоксов слоёв
    # (клиентский стейт SPA) при ней может сбрасываться.
    print(f"{prefix}[ПАУЗА] Откройте карточку. Если ещё не включены — включите на карте "
          f"чекбоксы слоёв «Здания» и «Объекты капитального строительства» (ОКС): "
          f"WFS/PKK их не требуют, но OL-state/CV-fallback видят только то, что "
          f"реально отрисовано. Дождитесь данных справа и нажмите ENTER...")
    try:
        input()
    except (EOFError, KeyboardInterrupt):
        raise UserExit()
