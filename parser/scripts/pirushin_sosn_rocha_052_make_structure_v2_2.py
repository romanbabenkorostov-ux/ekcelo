...
def load_enriched_extras(folder: Path) -> dict[str, Any]:
    ...
    Стратегия выбора файла (v2_2, hotfix parser-team A, PR #27):

      1) **canonical** `enriched.json` (v17 enricher, фиксированное имя) —
         если найден хоть один такой файл (rglob), используется ТОЛЬКО он.
      2) **legacy** `enriched_*.json` (v14 enricher, имя с timestamp) —
         как fallback, если canonical отсутствует. Перебираются все
         совпадения (на случай нескольких legacy-выгрузок).

    Зачем приоритет: при rglob без приоритезации `business_units` от
    canonical (v17) и от legacy (v14) АГРЕГИРОВАЛИСЬ в один список
    (`append`, не overwrite). Это давало дубли BU, если в `_data/`
    одновременно лежали `enriched.json` и `enriched_20260522_140555.json`.
    Покрыто тестами `test_052_load_enriched_priority_canonical` /
    `_fallback_to_legacy` от parser-team A.
    """
    ...
    legacy = [p for p in folder.rglob("enriched_*.json")
              if p.name != "enriched.json"]
    # Если canonical найден — игнорируем legacy (избегаем двойной агрегации).
    candidates = canonical if canonical else legacy
    # Любые другие *.json (например, NSPD-кеш с побочными beneficiaries)
    # рассматриваем дополнительно — но только если они НЕ являются
    # enriched-файлами, чтобы не пересечься с приоритезацией выше.
    for jp in folder.rglob("*.json"):
        if jp.name == "enriched.json":
            continue
        if jp.name.startswith("enriched_") and jp.name.endswith(".json"):
            continue
...
def link_with_enriched(
    ...
    #
    # ВАЖНО (v2.1): начиная с v17 enricher в `beneficiaries` могут
    # присутствовать ИП и ФЛ (новые `_kind = "ip"` и `"person"`).
    # Их ФИО не должны ошибочно матчиться с названием юр.лица —
    # фильтруем по `_kind` и `Тип субъекта`.
    candidate: dict | None = None
    target = (enterprise.get("name_short") or "").upper()
    for key, ben in beneficiaries.items():
        if not isinstance(ben, dict):
            continue
        # Фильтр: только ЮЛ. Пропускаем ИП и ФЛ.
        kind = ben.get("_kind")
        if kind in ("ip", "person"):
            continue
        subj_type = (ben.get("attrs", {}).get("Тип субъекта")
                     if isinstance(ben.get("attrs"), dict) else None)
        if subj_type in ("Индивидуальный предприниматель", "Физическое лицо"):
            continue
        ...
        # v17 хранит атрибуты также внутри ben["attrs"] — добавляем fallback
        if not full and isinstance(ben.get("attrs"), dict):
            full = (ben["attrs"].get("Полное наименование") or "").upper()
            short = (ben["attrs"].get("Краткое наименование") or "").upper()
    ...
    if candidate:
        ...
        enterprise["ogrn"] = candidate.get("ОГРН") or attrs.get("ОГРН") or enterprise.get("ogrn")
        enterprise["kpp"]  = candidate.get("КПП")  or attrs.get("КПП")  or enterprise.get("kpp")
