"""
03_enrich_v17.py — обогатитель данных об объектах недвижимости и бенефициарах.

Что делает:
  • Принимает на вход множество JSON/PDF из разных источников и сливает их
    в один enriched.json по кадастровым номерам и связям между ЮЛ/ИП/ФЛ.

Поддерживаемые источники (определяются автоматически):
  • nspd          — session_export_*.json из 01_parsing_nspd_*.py
  • kadbase       — final_<КН>.json из 022_parsing_pomech_kadastren_*.py
  • egrn          — выписки ЕГРН (tables-формат)
  • certificate   — свидетельства о собственности
  • kml           — геометрия объектов
  • legal_entity  — LEGAL_<ОГРН>_*.json (юридическое лицо)
  • ip_entity     — фл-выписка из ЕГРИП (LEGAL JSON или PDF)
  • person_entity — PERSON_<ИНН>_*.json (физлицо — справка ФНС о связях)
  • egrul_pdf     — ul-<ОГРН>-*.pdf (выписка ЕГРЮЛ из ФНС)
  • egrip_pdf     — fl-<ОГРНИП>-*.pdf (выписка ЕГРИП из ФНС)

Что обогащает:
  • Этажность ОКС — по «Этажу» помещений из связанных объектов.
  • Бизнес-единицы — по совпадению адресов бенефициара и объекта недвижимости
    (интерактивный диалог сопоставления, решения сохраняются в match_decisions.json).
  • Цепочки учредителей и руководителей (юр.лицо/ИП/физлицо → дочернее ЮЛ).
  • Обременения уставного капитала (залог доли + сведения о залогодержателе).
  • Для АО — держатель реестра акционеров.

Запуск:
    python 03_enrich_v17.py <file1> [<file2> ...]

Идемпотентность:
  • Выходные файлы в указанной папке имеют фиксированные имена
    (enriched.json, match_decisions.json) и перезаписываются на каждом запуске.
"""
import json
import re
import sys
...
# Опциональная зависимость для парсинга PDF-выписок ФНС (ЕГРЮЛ/ЕГРИП).
# Если pdfplumber не установлен — PDF-источники будут отклоняться с подсказкой.
try:
...
AREA_RE = re.compile(r"(\d[\d\s\u00a0]*(?:[.,]\d+)?)\s*(?:кв\.?\s*м|м²|кв\.\s*м\.?)")
...
# Поля, приходящие из 01_parsing_nspd v7 (session_export 25.0).
# Канонические имена выровнены с EGRN_BUILDING_FIELDS / KADBASE_FIELD_ALIASES.
NSPD_FIELD_ALIASES = {
    "Дата присвоения": "Дата постановки на учёт",
    "Площадь": "Площадь, кв.м",
    "Площадь общая": "Площадь, кв.м",
    "Площадь уточненная": "Площадь, кв.м",
    "Площадь застройки": "Площадь застройки, кв.м",
    "Количество этажей (в том числе подземных)": "Количество этажей всего",
    "Количество подземных этажей": "Этажей подземных",
    "Завершение строительства": "Год завершения строительства",
    "Ввод в эксплуатацию": "Год ввода в эксплуатацию",
    "Вид разрешенного использования": "Вид разрешённого использования",
    "Виды разрешенного использования": "Вид разрешённого использования",
}
...
NSPD_DROP_VALUES = {"-", "—", "–", "", "не указано", "нет данных"}
...
SOURCE_PRIORITY = {"egrn": 4, "nspd": 3, "kadbase": 2, "kml": 2, "certificate": 1,
                    "legal_entity": 3, "ip_entity": 3}
...
def detect_file_kind(data: Any, path: Path) -> str:
    ...
    if "entity" in data and isinstance(data.get("entity"), dict) and data.get("query"):
        ...
        ent_type = (data.get("entity") or {}).get("type")
        if src == "egrip_pdf" or ent_type == "ip":
            return "ip_entity"
        if ent_type == "person":
...
def _nspd_extract_int(value: Any) -> int | None:
    """Достаёт первое целое число из значения ('2019 г.' → 2019, '80%' → 80)."""
    if value is None:
        return None
...
def _nspd_normalize_attrs(raw: dict) -> dict:
    """
    Нормализует атрибуты из NSPD v7 session_export:
      - переименовывает поля по NSPD_FIELD_ALIASES;
      - вытаскивает число из «Площадь, кв.м» (с разрядными пробелами);
      - приводит к int поля этажей и годов (NSPD_INT_FIELDS);
      - выбрасывает строковые пустышки ('-', '—', '–', '', 'не указано', 'нет данных').
    Числовые поля кадастровой стоимости NSPD v7 уже отдаёт float — не трогаем.
    Конфликт ключей (например «Площадь» + «Площадь общая»): побеждает первый
    непустой исходный ключ в порядке итерации.
    """
    out: dict = {}
    for raw_key, raw_val in raw.items():
        if raw_val is None:
            continue
        if isinstance(raw_val, str) and raw_val.strip() in NSPD_DROP_VALUES:
            continue

        key = NSPD_FIELD_ALIASES.get(raw_key, raw_key)

        if key in ("Площадь, кв.м", "Площадь застройки, кв.м"):
            num = extract_area(str(raw_val))
            if num is None:
        ...
        if key in NSPD_INT_FIELDS:
            num = _nspd_extract_int(raw_val)
            if num is not None:
                out.setdefault(key, num)
            continue
...
def parse_nspd(data: dict, source_id: str) -> tuple[dict, list, list]:
    ...
    for category, items in root.items():
        ...
        for cn, attrs in items.items():
            ...
            attrs_clean_raw = {k: v for k, v in attrs.items() if k != "Связанные объекты"}
            geom = maybe_extract_geometry(attrs_clean_raw)
            attrs_clean = _nspd_normalize_attrs(attrs_clean_raw)
            obj_rec = {
...
def parse_person_entity(data: dict, source_id: str) -> tuple[dict, list]:
    """PERSON-выписка (entity.type='person') → бенефициар + список связей founder→child.

    Извлекаются:
      • ФИО, ИНН-12, статус
      • Где руководитель  (raw_data.Руковод) — список ЮЛ
      • Где учредитель    (raw_data.Учред)   — список ЮЛ
      • ИП-привязки       (raw_data.ИП)      — список ОГРНИП
    Каждая связь учредитель→ЮЛ возвращается в founders_links,
    чтобы попасть в build_founder_chains как связь person → child_legal.
    """
    ...
    inn = entity.get("inn")
    fio = entity.get("fio") or entity.get("name_full") or ""
    ogrnip = entity.get("ogrnip")

    def _legal_brief(rec: dict) -> dict:
        return {
            "ОГРН": rec.get("ОГРН"),
            "ИНН": rec.get("ИНН"),
            "Наименование": rec.get("НаимСокр") or rec.get("НаимПолн"),
            "Статус": rec.get("Статус"),
            "Дата ликвидации": rec.get("ДатаЛикв"),
            "Юр. адрес": rec.get("ЮрАдрес"),
            "ОКВЭД": rec.get("ОКВЭД"),
        }

    director_in = [_legal_brief(x) for x in (raw.get("Руковод") or [])]
    founder_in = [_legal_brief(x) for x in (raw.get("Учред") or [])]
    ip_list = list(raw.get("ИП") or [])

    # Связи "это физлицо — учредитель такого-то ЮЛ" → для построения цепочек.
    founders_links = []
    for u in (raw.get("Учред") or []):
        if u.get("Статус") and "не действует" in str(u["Статус"]).lower():
            continue  # пропускаем ликвидированные дочерние компании
        founders_links.append({
            "kind": "person_to_legal",
            "child_ogrn": u.get("ОГРН"),
            "child_inn": u.get("ИНН"),
            "child_name": u.get("НаимСокр") or u.get("НаимПолн"),
            "child_status": u.get("Статус"),
        })
    ...
    entity_attrs = {
        ...
        "ИНН": inn,
        "ОГРНИП": ogrnip,
        "Пол": entity.get("gender"),
        "Статус": entity.get("status"),
        "Действует": entity.get("is_active"),
        "Самозанятый": entity.get("is_self_employed"),
        "Является руководителем (количество)": len(director_in) or None,
        "Является руководителем (организации)": director_in or None,
        "Является учредителем (количество)": len(founder_in) or None,
        "Является учредителем (организации)": founder_in or None,
        "Зарегистрирован ИП": ip_list or None,
    }
    entity_attrs = {k: v for k, v in entity_attrs.items() if v not in (None, "", [], {})}

    bkey = beneficiary_key_from_legal(None, inn, fio) if inn else f"person::{stable_hash(fio.lower())}"
    return {
        bkey: {
            "_kind": "person",
            "_ogrn": None,
...
def parse_ip_entity(data: dict, source_id: str) -> tuple[dict, list]:
    """ЕГРИП-выписка (entity.type='ip') → бенефициар вида 'Индивидуальный предприниматель'.
    Возвращает (beneficiaries_dict, founders_links_list=[])."""
    entity = data.get("entity") or {}
    if not entity:
        return {}, []

    ogrnip = entity.get("ogrnip") or entity.get("ogrn")
    inn = entity.get("inn")
    fio = entity.get("name_full") or entity.get("name_short") or ""
    ...
    entity_attrs = {
        ...
        "ИНН": inn,
        "Пол": entity.get("gender"),
        "Гражданство": entity.get("citizenship"),
        "Электронная почта": entity.get("email"),
        "Дата регистрации": entity.get("reg_date"),
        "Налоговый орган": entity.get("tax_organ"),
        "Статус": entity.get("status"),
        "Действует": entity.get("is_active"),
    ...
    bkey = beneficiary_key_from_legal(ogrnip, inn, fio)
    return {
        bkey: {
            "_kind": "ip",
            "_ogrn": ogrnip,
            ...
            "_kpp": None,
            "_name": fio,
            "_addresses": [],  # у ИП адрес не публикуется
            "_sources": [source_id],
            "_founders_links": [],
            "attrs": entity_attrs,
        }
...
def parse_legal_entity(data: dict, source_id: str) -> tuple[dict, list]:
    ...
    pledges = []  # обременения долей УК (залог)
    ...
    for fr in (raw.get("Учред", {}) or {}).get("РосОрг", []) or []:
        rec = {
            ...
            "Доля, руб.": (fr.get("Доля") or {}).get("Номинал"),
        }
        # Обременение доли (залог)
        obrems = fr.get("Обрем") or []
        if obrems:
            rec["Обременения доли"] = []
            for ob in obrems:
                holder = ob.get("Залогодерж") or {}
                notar = ob.get("Нотариал") or {}
                pledge_item = {
                    "Вид": ob.get("Тип", "Залог"),
                    "Залогодержатель": {
                        "ОГРН": holder.get("ОГРН"),
                        "ИНН": holder.get("ИНН"),
                        "Наименование": holder.get("НаимПолн") or holder.get("НаимСокр"),
                    },
                    "Договор залога": {
                        "Номер": notar.get("Номер"),
                        "Дата": notar.get("Дата"),
                    },
                    "Нотариус": ob.get("НотариусФИО") or notar.get("ФИО"),
                }
                pledge_item = {k: v for k, v in pledge_item.items() if v not in (None, "", {})}
                rec["Обременения доли"].append(pledge_item)
                pledges.append({
                    "Доля учредителя": fr.get("НаимПолн") or fr.get("НаимСокр") or fr.get("ОГРН"),
                    **pledge_item,
                })
    ...
    # Держатель реестра акционеров (только для АО)
    register_holder = None
    drh = raw.get("ДержРеестрАО") or {}
    if isinstance(drh, dict) and (drh.get("ОГРН") or drh.get("ИНН")):
        register_holder = {
            "ОГРН": drh.get("ОГРН"),
            "ИНН": drh.get("ИНН"),
            "Наименование": drh.get("НаимПолн") or drh.get("НаимСокр"),
        }
    ...
    entity_attrs = {
        ...
        "Дата регистрации": entity.get("reg_date"),
        "Уставный капитал, руб.": entity.get("ust_kap"),
        "Основной ОКВЭД": okved_main_disp,
        "Дополнительные ОКВЭД": okved_additional_disp or None,
        ...
        "Обременения долей в УК": pledges or None,
        "Держатель реестра акционеров": register_holder,
...
def load_file(path: Path) -> dict:
    ...
    if path.suffix.lower() == ".pdf":
        if not _HAVE_PDFPLUMBER:
            raise RuntimeError(
                "Для парсинга PDF-выписок ФНС нужен пакет pdfplumber. "
                "Установите: pip install pdfplumber"
            )
        return parse_fns_pdf(path)
    return json.loads(path.read_text(encoding="utf-8"))


# ──────────────────────────────────────────────────────────────────────────
#                      ЕГРЮЛ/ЕГРИП PDF — парсер (встроенный)
# ──────────────────────────────────────────────────────────────────────────


# Глобальные «закрыватели» блока учредителя/директора.
# Когда встречаем одно из этих полей — закрываем открытый блок и трактуем
# дальнейшее как поле верхнего уровня компании.
_FNS_BLOCK_BREAKERS = (
    "Размер (в рублях)",
...
def _fns_extract_rows(pdf_path):
    """Возвращает плоский список (num_or_None, field, value) со всех страниц."""
    rows = []
        ...
        for page in pdf.pages:
            for table in page.extract_tables() or []:
                for row in table:
                    if len(row) != 3:
                        continue
                    num_raw, field, value = row
                    n = (num_raw or "").strip()
                    f = re.sub(r"\s+", " ", (field or "").strip())
                    v = (value or "").strip()
                    if not f and not v:
                        continue
                    rows.append((int(n) if n.isdigit() else None, f, v))
    return rows


def _fns_detect_kind(pdf_path):
    """'egrul' | 'egrip' | None — по тексту первой страницы."""
    with pdfplumber.open(str(pdf_path)) as pdf:
        head = (pdf.pages[0].extract_text() or "").lower()
    if "реестра юридических лиц" in head:
        return "egrul"
    if "реестра индивидуальных предпринимателей" in head:
        return "egrip"
    return None


def _fns_to_iso_date(s):
...
def _fns_parse_decimal(s):
    s = re.sub(r"[^\d.,\-]", "", s or "").replace(",", ".").strip(".")
    if not s:
        return None
    try:
        return float(s)
...
def _fns_split_fio(value):
    """'ФАМИЛИЯ\\nИМЯ\\nОТЧЕСТВО' → 'Фамилия Имя Отчество' (title-case)."""
    parts = [p.strip() for p in re.split(r"[\n\r]+|\s{2,}", value) if p.strip()]
    if len(parts) >= 2:
        normed = [(p[:1].upper() + p[1:].lower() if p else p) for p in parts[:3]]
        return " ".join(normed)
    return value.strip()


def _fns_egrul_to_dict(rows, full_text=""):
    """Сводит строки таблиц ЕГРЮЛ-выписки в entity-словарь, совместимый с LEGAL JSON.

    Извлекает:
      • основные поля компании, директора, учредителей;
      • обременения долей в УК (залог + залогодержатель + нотариус);
      • держателя реестра акционеров — через regex по полному тексту
        (если передан full_text), так как в табличном потоке у этой секции
        нет надёжного маркера-открывателя.
    """
    entity = {
        "type": "legal",
        "is_active": True,
    ...
    raw_uchred = {"ФЛ": [], "РосОрг": [], "ИнОрг": [], "ПИФ": [], "РФ": []}
    raw_data = {"Учред": raw_uchred, "Руковод": []}

    seen_global = set()
    cur_block = None
    okved_main_taken = False

    # Состояние подблока «обременение доли». Возможные значения:
    #   None                       — не в залоге
    #   'pledge_terms'             — открыт залог, ждём номер/дату/нотариуса
    #   'pledge_holder_expected'   — увидели «Сведения о залогодержателе»; следующий
    #                                «ГРН ... о данном лице» — это начало holder-блока
    #   'pledge_holder_open'       — внутри holder-блока, ОГРН/ИНН/НаимПолн → в holder
    sub_state = None
    cur_pledge = None
    cur_holder = None

    def open_block():
        nonlocal cur_block, sub_state, cur_pledge, cur_holder
        flush_pledge()
        flush_block()
        cur_block = {}
        sub_state = None
    ...
    def flush_pledge():
        nonlocal cur_pledge, cur_holder, sub_state
        if cur_pledge is not None and cur_block is not None:
            if cur_holder:
                cur_pledge["holder"] = cur_holder
            cur_block.setdefault("pledges", []).append(cur_pledge)
        cur_pledge = None
        cur_holder = None
        sub_state = None
    ...
    def flush_block():
        ...
        if not cur_block:
            cur_block = None
            return
        has_share = ("share_nom" in cur_block) or ("share_percent" in cur_block)
        has_position = "position" in cur_block
        pledges = cur_block.get("pledges") or []
        if has_position:
            raw_data["Руковод"].append({
                "ФИО": cur_block.get("ФИО"),
                "ИНН": cur_block.get("ИНН"),
                "НаимДолжн": cur_block.get("position"),
            })
            entity["directors"].append({
                "fio": cur_block.get("ФИО"),
                "inn": cur_block.get("ИНН"),
                "position": cur_block.get("position"),
            })
        elif has_share and cur_block.get("ОГРН"):
            obrem_list = []
            for p in pledges:
                ...
                obrem_list.append({
                    ...
                    "Залогодерж": {
                        "ОГРН": holder.get("ОГРН"),
                        "ИНН": holder.get("ИНН"),
                        "НаимПолн": holder.get("НаимПолн"),
                    },
                    "Нотариал": {
                        "Номер": p.get("contract_number"),
                        "Дата": p.get("contract_date"),
                    },
                    "НотариусФИО": p.get("notary_fio"),
                })
            ros = {
                "ОГРН": cur_block.get("ОГРН"),
                "ИНН": cur_block.get("ИНН"),
                "НаимПолн": cur_block.get("НаимПолн"),
                "Доля": {"Номинал": cur_block.get("share_nom"),
                         "Процент": cur_block.get("share_percent")},
            }
            if obrem_list:
                ros["Обрем"] = obrem_list
            raw_uchred["РосОрг"].append(ros)
            entity["founders"].append({
                "ogrn": cur_block.get("ОГРН"),
                "inn": cur_block.get("ИНН"),
                "name": cur_block.get("НаимПолн"),
                "share_nom": cur_block.get("share_nom"),
                "share_percent": cur_block.get("share_percent"),
                "entity_type": "legal",
                "has_pledge": bool(obrem_list),
            })
        elif has_share and cur_block.get("ФИО"):
            raw_uchred["ФЛ"].append({
                "ФИО": cur_block.get("ФИО"),
                "ИНН": cur_block.get("ИНН"),
                "Доля": {"Номинал": cur_block.get("share_nom"),
                         "Процент": cur_block.get("share_percent")},
    ...
    for n, f, v in rows:
        ...
        # ── подблоки залога доли (учредителя) ──
        if "сведения об обременении" in fl and not v:
            # маркер «начался блок обременения текущего учредителя»
            if cur_block is not None:
                sub_state = "pledge_terms"
                cur_pledge = {}
                cur_holder = None
            continue
        if "сведения о договоре залога" in fl:
            if cur_block is None or cur_pledge is None:
                cur_pledge = cur_pledge or {}
                sub_state = sub_state or "pledge_terms"
            v_one = re.sub(r"\s+", " ", v).strip()
            m_num = re.search(r"номер договора залога:\s*(.+?)\s*(?:дата договора залога:|$)",
                              v_one, re.IGNORECASE)
            m_date = re.search(r"дата договора залога:\s*([\d.]+)", v_one, re.IGNORECASE)
            if m_num:
                cur_pledge["contract_number"] = re.sub(r"\s+", "", m_num.group(1).strip())
            if m_date:
                cur_pledge["contract_date"] = _fns_to_iso_date(m_date.group(1).strip())
            continue
        if "сведения о нотариусе" in fl and "договор залога" in fl:
            if cur_pledge is not None:
                cur_pledge["notary_fio"] = _fns_split_fio(v)
            continue
        if "сведения о залогодержателе" in fl and not v:
            # следующий маркер «ГРН … о данном лице» — начало holder-блока
            sub_state = "pledge_holder_expected"
            cur_holder = {}
            continue

        if "грн и дата внесения в егрюл сведений о данном лице" in fl:
            if sub_state == "pledge_holder_expected":
                sub_state = "pledge_holder_open"
                cur_holder = {}
                continue
            # обычный путь: новый блок учредителя/директора
            open_block()
            continue

        if any(brk in f for brk in _FNS_BLOCK_BREAKERS):
            flush_pledge()
            flush_block()

        # ── глобальные поля компании ──
        if "полное наименование на русском языке" in fl:
            val = take_global("name_full", v.replace("\n", " ").strip())
            if val: entity["name_full"] = val
            continue
        if "сокращенное наименование на русском языке" in fl:
            val = take_global("name_short", v.replace("\n", " ").strip())
            if val: entity["name_short"] = val
            continue
        if "адрес юридического лица" in fl:
            val = take_global("address", re.sub(r"\s+", " ", v).rstrip(", "))
            if val: entity["address"] = val
            continue
        if "место нахождения юридического лица" in fl:
            val = take_global("region", v.strip())
            if val: entity["region"] = val
            continue
        if f.strip() == "ОГРН" and cur_block is None and sub_state != "pledge_holder_open":
            val = take_global("ogrn", v.replace(" ", "").strip())
            if val: entity["ogrn"] = val
            continue
        if "дата присвоения огрн" in fl or (f.strip() == "Дата регистрации" and cur_block is None):
            val = take_global("reg_date", _fns_to_iso_date(v))
            if val: entity["reg_date"] = val
            continue
        if "инн юридического лица" in fl:
            entity["inn"] = v.replace(" ", "").strip()
            continue
        if "кпп юридического лица" in fl:
            entity["kpp"] = v.replace(" ", "").strip()
            continue
        if "размер (в рублях)" in fl:
            num = _fns_parse_decimal(v)
            if num is not None:
                entity["ust_kap"] = int(num)
            continue
        if "код и наименование вида деятельности" in fl:
            m = re.match(r"([\d.]+)\s+(.+)", v.replace("\n", " "))
            if m:
                item = {"code": m.group(1), "name": m.group(2).strip()}
                if not okved_main_taken:
                    entity["okved_main"] = item
                    okved_main_taken = True
                else:
                    entity["okved_additional"].append(item)
            continue

        # ── поля внутри holder-блока залогодержателя ──
        if sub_state == "pledge_holder_open":
            if f.strip() == "ОГРН":
                cur_holder["ОГРН"] = v.replace(" ", "").strip()
            elif f.strip() == "ИНН":
                cur_holder["ИНН"] = v.replace(" ", "").strip()
            elif "полное наименование" in fl:
                cur_holder["НаимПолн"] = v.replace("\n", " ").strip()
            continue

        # ── поля внутри блока (директор/учредитель) ──
        if cur_block is not None:
            if "фамилия" in fl and "имя" in fl and "отчество" in fl:
                cur_block["ФИО"] = _fns_split_fio(v)
            elif f.strip() == "ИНН":
                cur_block["ИНН"] = v.replace(" ", "").strip()
            elif f.strip() == "ОГРН":
                cur_block["ОГРН"] = v.replace(" ", "").strip()
            elif "полное наименование" in fl:
                cur_block["НаимПолн"] = v.replace("\n", " ").strip()
            elif "должность" in fl:
                cur_block["position"] = v.strip()
            elif "номинальная стоимость доли" in fl:
                num = _fns_parse_decimal(v)
                if num is not None: cur_block["share_nom"] = num
            elif "размер доли" in fl:
                num = _fns_parse_decimal(v)
                if num is not None: cur_block["share_percent"] = num
            continue

    flush_pledge()
    flush_block()

    # Держатель реестра акционеров (только для АО) — извлекаем regex'ом по полному тексту.
    if full_text:
        m_sec = re.search(
            r"Сведения о держателе реестра акционеров[^\n]*\n(.*?)(?:\n\s*Сведения о|\n\s*Сведения об учете)",
            full_text, re.DOTALL,
        )
        if m_sec:
            sec = m_sec.group(1)
            m_ogrn = re.search(r"\bОГРН\s+(\d{13})\b", sec)
            m_inn = re.search(r"\bИНН\s+(\d{10})\b", sec)
            m_name = re.search(
                r"Полное наименование\s+([^\n]+(?:\n[^\n0-9]+)*)",
                sec,
            )
            holder = {}
            if m_ogrn: holder["ОГРН"] = m_ogrn.group(1)
            if m_inn: holder["ИНН"] = m_inn.group(1)
            if m_name:
                name_text = re.sub(r"\s+", " ", m_name.group(1)).strip()
                # обрезаем по «ГРН и дата» — это уже следующая строка
                name_text = re.split(r"\s+ГРН\b", name_text)[0].strip()
                holder["НаимПолн"] = name_text
            if holder:
                raw_data["ДержРеестрАО"] = holder
    return {
        "query": {"type": "legal", "value": entity.get("ogrn")},
        "entity": entity,
        "raw_data": raw_data,
        "_source": "egrul_pdf",
    }
...
def _fns_egrip_to_dict(rows):
    """ЕГРИП-выписка → entity-словарь (entity.type='ip')."""
    entity = {
        "type": "ip",
        "is_active": True,
        "status": "Действует",
    ...
    for n, f, v in rows:
        fl = f.lower()
        if "фамилия" in fl and "имя" in fl and "отчество" in fl:
            val = take("fio", _fns_split_fio(v))
            if val:
                entity["name_full"] = val
                entity["name_short"] = val
                parts = val.split()
                if len(parts) >= 1: entity["last_name"] = parts[0]
                if len(parts) >= 2: entity["first_name"] = parts[1]
                if len(parts) >= 3: entity["patronymic"] = " ".join(parts[2:])
            continue
        if f.strip() == "Пол":
            val = take("gender", v.strip().lower())
            if val: entity["gender"] = val
            continue
        if f.strip() == "Гражданство":
            val = take("citizenship", v.strip())
            if val: entity["citizenship"] = val
            continue
        if f.strip() == "E-mail" or "адрес электронной почты" in fl:
            val = take("email", v.strip())
            if val: entity["email"] = val
            continue
        if f.strip() == "ОГРНИП":
            val = take("ogrnip", v.replace(" ", "").strip())
            if val: entity["ogrnip"] = val
            continue
        if f.strip() == "Дата регистрации":
            val = take("reg_date", _fns_to_iso_date(v))
            if val: entity["reg_date"] = val
            continue
        if "идентификационный номер налогоплательщика" in fl or f.strip() == "ИНН":
            val = take("inn", v.replace(" ", "").strip())
            if val: entity["inn"] = val
            continue
        if "наименование налогового органа" in fl:
            val = take("tax_organ", v.strip())
            if val: entity["tax_organ"] = val
            continue
        if "код и наименование вида деятельности" in fl:
            m = re.match(r"([\d.]+)\s+(.+)", v.replace("\n", " "))
            if m:
    ...
    return {
        "query": {"type": "ip", "value": entity.get("ogrnip")},
        "entity": entity,
        "raw_data": {},
        "_source": "egrip_pdf",
    }
...
def parse_fns_pdf(path):
    """Парс PDF-выписки ФНС → LEGAL-подобный dict (с entity и raw_data)."""
    kind = _fns_detect_kind(path)
    rows = _fns_extract_rows(path)
    if kind == "egrul":
        with pdfplumber.open(str(path)) as pdf:
            full_text = "\n".join((p.extract_text() or "") for p in pdf.pages)
        return _fns_egrul_to_dict(rows, full_text=full_text)
    if kind == "egrip":
        return _fns_egrip_to_dict(rows)
    raise ValueError(f"Не удалось определить тип ФНС-выписки: {path}")
...
def enrich_floors_from_premises(merged_objects: dict) -> int:
    """
    Если у ОКС (Здание/Сооружение/ОНС) нет «Количество этажей всего» — выводим
    из максимума «Этаж» по помещениям, перечисленным в «Связанные объекты».

    Учитывает все варианты исходных имён ключа этажности (NSPD, EGRN, kadbase):
      - «Количество этажей всего»               (EGRN, NSPD после нормализации)
      - «Количество этажей»                     (kadbase / historical NSPD)
      - «Количество этажей (в том числе подземных)» (исходный NSPD до v15)
      - «Этажей надземных»                      (EGRN)

    Результат пишется в канонические EGRN-имена:
      - «Количество этажей всего»
      - «Этажей подземных»
    ...
    for cn, obj in merged_objects.items():
        ...
        # Уже есть нормальная этажность — не трогаем
        if _has_meaningful(ABOVE_KEYS):
            continue
        ...
        if max_above or max_below:
            if max_above:
                attrs["Количество этажей всего"] = max_above
            if max_below and not _has_meaningful(BELOW_KEYS):
                attrs["Этажей подземных"] = max_below
...
def _print_legal_beneficiaries(payload: dict):
    """Печатает руководителей, учредителей, держателя реестра АО, обременения долей."""
    if not payload:
        return
    attrs = payload.get("attrs", {})

    for d in attrs.get("Руководители", []) or []:
        fio = d.get("ФИО") or "?"
        inn = d.get("ИНН") or "?"
        pos = d.get("Должность") or "руководитель"
        print(f"    [Р] {pos}: {fio}  (ИНН {inn})")

    for fr in attrs.get("Учредители (юр.лица и публично-правовые)", []) or []:
        name = fr.get("Наименование") or "?"
        inn = fr.get("ИНН") or "?"
        share = fr.get("Доля, %")
        share_str = f", доля {share}%" if share is not None else ""
        print(f"    [У] учредитель: {name}  (ИНН {inn}){share_str}")
        for ob in fr.get("Обременения доли", []) or []:
            holder = ob.get("Залогодержатель", {}) or {}
            h_name = holder.get("Наименование") or "?"
            h_inn = holder.get("ИНН") or "?"
            contract = ob.get("Договор залога", {}) or {}
            print(f"        [!] ЗАЛОГ ДОЛИ → {h_name}  (ИНН {h_inn})")
            if contract.get("Номер"):
                print(f"            договор № {contract.get('Номер')} от {contract.get('Дата','?')}")

    pers_cnt = attrs.get("Учредителей — физических лиц")
    if pers_cnt:
        print(f"    [У] учредителей — физических лиц: {pers_cnt}")

    drh = attrs.get("Держатель реестра акционеров")
    if drh:
        d_name = drh.get("Наименование") or "?"
        d_inn = drh.get("ИНН") or "?"
        print(f"    [АО] держатель реестра акционеров: {d_name}  (ИНН {d_inn})")


def _print_person_links(payload: dict, show_cross_links: bool = True):
    """Печатает связи физлица: где руководит, где учредитель.
    Если show_cross_links=False — только сводные числа, без перечисления ЮЛ."""
    if not payload:
        return
    attrs = payload.get("attrs", {})
    director_in = attrs.get("Является руководителем (организации)", []) or []
    founder_in = attrs.get("Является учредителем (организации)", []) or []
    ip_list = attrs.get("Зарегистрирован ИП", []) or []

    def _fmt_org_line(org: dict) -> str:
        name = org.get("Наименование") or "?"
        inn = org.get("ИНН") or "?"
        status = org.get("Статус") or ""
        mark = " (ликв.)" if "не действует" in str(status).lower() else ""
        return f"        • {name}  (ИНН {inn}){mark}"

    if director_in:
        print(f"    [Р] руководитель в {len(director_in)} ЮЛ"
              + (":" if show_cross_links else ""))
        if show_cross_links:
            for org in director_in:
                print(_fmt_org_line(org))

    if founder_in:
        print(f"    [У] учредитель в {len(founder_in)} ЮЛ"
              + (":" if show_cross_links else ""))
        if show_cross_links:
            for org in founder_in:
                print(_fmt_org_line(org))

    if ip_list:
        print(f"    [ИП] зарегистрирован как ИП: {len(ip_list)}")


def ask_show_cross_links() -> bool:
    """Спрашивает: показывать ли кросс-связи руководителей/учредителей с другими ЮЛ."""
    try:
        ans = input("Показывать связи руководителей/учредителей с другими бизнесами? [Y/n]: ").strip().lower()
    except EOFError:
        return True
...
def build_founder_chains(beneficiaries: dict) -> list[dict]:
    ...
    for b_key, b_data in list(beneficiaries.items()):
        for link in b_data.get("_founders_links", []) or []:
            # PERSON JSON: сам бенефициар (b_key) является учредителем child-ЮЛ.
            # Связь идёт: b_key (founder/person) → child (ЮЛ из link.child_*).
            if link.get("kind") == "person_to_legal":
                ogrn = link.get("child_ogrn")
                inn = link.get("child_inn")
                name = link.get("child_name")
                status = link.get("child_status")
                if ogrn:
                ...
                if child_key not in beneficiaries:
                    beneficiaries[child_key] = {
                        ...
                        "attrs": {k: v for k, v in {
                            "Тип субъекта": "Юридическое лицо (упомянуто как дочернее ФЛ)",
                            "ОГРН": ogrn,
                            "ИНН": inn,
                            "Наименование": name,
                            "Статус": status,
                ...
                edge_key = (b_key, child_key, "person_to_legal")
                if edge_key in seen:
                    continue
                seen.add(edge_key)
                chains.append({
                    ...
                    "child_key": child_key,
                    "share_percent": None,
                    "context": "Учредитель — физическое лицо (по PERSON-выписке)",
                    "kind": "person_to_legal",
                })
                continue
...
def beneficiary_display_name(payload: dict) -> str:
    ...
    if payload.get("_kind") == "ip":
        attrs = payload.get("attrs", {})
        fio = attrs.get("ФИО") or payload.get("_name")
        return f"ИП {fio}" if fio else "индивидуальный предприниматель"
    attrs = payload.get("attrs", {})
...
def print_banner():
    print("=" * 70)
    print(" Обогатитель данных об объектах недвижимости и бенефициарах  v17")
    print("=" * 70)
    print(" Поддерживаемые источники:")
    print("   • NSPD session_export   — объекты с НСПД")
    print("   • Kadbase final_<КН>    — данные с kadbase/rosreestr/egrp365")
    print("   • EGRN tables-выписки   — данные ЕГРН")
    print("   • Свидетельства собств. — JSON-формат")
    print("   • KML                   — геометрия")
    print("   • LEGAL_<ОГРН>.json     — карточка ЮЛ (ЕГРЮЛ)")
    print("   • LEGAL_<ОГРНИП>.json   — карточка ИП (ЕГРИП)")
    print("   • PERSON_<ИНН>.json     — карточка ФЛ (связи руковод./учред.)")
    print("   • ul-<ОГРН>-*.pdf       — PDF-выписка ЕГРЮЛ")
    print("   • fl-<ОГРНИП>-*.pdf     — PDF-выписка ЕГРИП")
    print("-" * 70)
    print(" Выходные файлы (в указанной папке, перезаписываются — идемпотентно):")
    print("   • enriched.json         — итоговый сводник")
    print("   • match_decisions.json  — сохранённые решения сопоставления")
    print("=" * 70)
    print()


def ask_output_dir() -> Path:
    """Запрашивает у пользователя папку для результатов. По умолчанию — текущая."""
    default = Path.cwd()
    try:
        raw = input(f"Папка для результатов [Enter = {default}]: ").strip().strip('"').strip("'")
    except EOFError:
        raw = ""
    if not raw:
        target = default
...
def main():
    ...
    show_cross = ask_show_cross_links()
    print()
    ...
    for path in paths:
        ...
        if kind == "nspd":
            ...
            src_label = "egrul_pdf" if data.get("_source") == "egrul_pdf" else "legal_entity"
            print(f"[+] {path.name} → формат: {src_label}, юрлицо: {name}, учредителей: {len(founders)}")
            _print_legal_beneficiaries(legal[next(iter(legal))] if legal else {})
        elif kind == "ip_entity":
            legal, founders = parse_ip_entity(data, source_id)
            for k, v in legal.items():
                if k in known_legal:
                    known_legal[k]["_sources"].extend(v.get("_sources", []))
                else:
                    known_legal[k] = v
            name = beneficiary_display_name(legal[next(iter(legal))]) if legal else "?"
            src_label = "egrip_pdf" if data.get("_source") == "egrip_pdf" else "ip_entity"
            print(f"[+] {path.name} → формат: {src_label}, ИП: {name}")
        elif kind == "person_entity":
            legal, founders = parse_person_entity(data, source_id)
            for k, v in legal.items():
                if k in known_legal:
                    known_legal[k]["_sources"].extend(v.get("_sources", []))
                    known_legal[k].setdefault("_founders_links", []).extend(v.get("_founders_links", []))
                    ...
                    known_legal[k] = v
            name = beneficiary_display_name(legal[next(iter(legal))]) if legal else "?"
            print(f"[+] {path.name} → формат: person_entity, ФЛ: {name}")
            _print_person_links(legal[next(iter(legal))] if legal else {}, show_cross_links=show_cross)
    ...
    decisions_path = output_dir / "match_decisions.json"
