...
def cn_to_id_part(cn: str) -> str:
    r"""КН → токен для использования в KML id (Style id, styleUrl якорь).

    `:` → `_`, `/` → `__` (часть КН).

    Контракт 2.11.0 §6 регламентирует КН в `<name>`/`description`
    (`\b\d{2}:\d{2}:\d{2,8}:\d{1,8}(?:/\d+)?\b`) и graph_node_id
    (`^[A-Za-z0-9_:/-]{1,256}$`). Эта функция — только для **внутренних
    KML id** (Style id и styleUrl-якорь), где `/` ломает фрагмент URL и
    делает невалидную ссылку. Сам КН в `<name>` остаётся в канонической
    форме `XX:XX:XXXXXXXX:XX[/N]`.

    Конвенция совместима с `_cn_to_mask` в 07_init_project и
    `CN_MASK_RE` в md 052 §2.7a (имена файлов / папок проекта).

    Примеры:
      61:44:0050706:31     → 61_44_0050706_31
      90:25:020102:119/1   → 90_25_020102_119__1
    """
