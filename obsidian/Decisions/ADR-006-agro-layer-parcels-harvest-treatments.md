...
(ADR-005 §E, реализовано `agro_link.link_parcel_to_land`), а будущая оценочная
вьюха агрегирует: контур (земля) × насаждение (сорт/возраст/кусты) × уход
(agro_event) × урожай (harvest). Слой оценки/погоды/почвы — отдельная итерация
(не в этом ADR), здесь фиксируется только связь и источник признаков.
