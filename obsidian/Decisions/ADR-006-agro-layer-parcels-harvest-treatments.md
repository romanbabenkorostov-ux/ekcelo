...
### J. Перечень виноградных насаждений (залоговый/оценочный источник) — DECIDED (2026-06-09)

Заказчик дал второй источник насаждений — текстовый **перечень из залоговых
документов**: блоки «Многолетние насаждения (виноградные насаждения), расположенные
на Предмете залога N…» с полями: фед. реестровый № насаждения, площадь насаждения,
сорт привоя, код сорта (госреестр селекционных достижений), площадь сорта, год
высадки, сорт подвоя, число кустов. Плюс табличные формы (Сорт | га | год посадки |
КН ЗУ | присвоенный № участка | схема посадки | кустов/га | всего кустов) и
залоговый блок (застраховано/заложено/балансовая/залоговая стоимость/тип/площадь).

**Маппинг (реализовано `vineyard_perechen.py`, `source='perechen'`):**
- блок насаждения → `agro_parcel` (parcel_code=`ПЗ-N`, `area_ha`, `land_cad`=КН ЗУ
  при наличии, расширенные поля в `attrs`: federal_reg_no, variety_code, rootstock,
  vines_count, area_variety_ha, pledge_item);
- сорт-привой + год высадки → `agro_crop_cycle` (виноград, `perennial`,
  `variety`, `sow_date`=год высадки, `crop_status='fact'`).
- `source='perechen'` добавлен в enum источников §6 (миграция 0005).

**Ценообразующая семантика (зачем это в модели).** Сорт (привой/подвой), год
высадки, число кустов, схема посадки — характеристики **насаждения на поверхности
контура ЗУ** (ADR-005): геопривязка + тип почвы + агротехнические свойства почв +
климат + накопленные погодные условия с момента посадки. Эти признаки —
ценообразующие: при должном уходе насаждение даёт урожай определённого качества.
Поэтому `agro_parcel`/`agro_crop_cycle` мягко привязываются к `land_contours`
(ADR-005 §E, реализовано `agro_link.link_parcel_to_land`).

**Реализовано (оценка + погода):** вьюха `v_vineyard_valuation`
(`0009`/`0010`, `agro_reports.vineyard_valuation`) агрегирует контур (земля:
площадь/центроид) × насаждение (сорт/возраст/кусты/фед.реестр) × уход (agro_event:
операции/обработки) × погода (`accum_gdd`/`accum_precip_mm`/`accum_radiation_mj`).
Погода — `weather_open_meteo.py` (Open-Meteo Archive, без ключа): накопленные с года
посадки GDD/осадки/радиация по геоточке центроида, снимок в `weather_accumulated`
(`0010`). **Остаётся (внешние источники):** урожай (harvest по сборам), тип почвы/
агротех-нормы, сетевой прогон погоды (offline-ядро готово).
