...
- Тесты: всё агро+земля → **64 passed**.

## ⏳ В плане (не сделано)

### P1 — расширение (по мере поступления карт)
- **Сорта/урожай:** перечень даёт сорт/возраст/площадь/кусты (статика насаждения);
  для урожая нужны карты со **сборами** (harvest: variety/volume_kg/acidity) —
  наполнят `v_agro_harvest_by_variety`/`v_agro_harvest_timing`.
- **Оценочная вьюха (ADR-006 §J):** контур(земля: почва/климат/погода) × насаждение
  (сорт/возраст/кусты) × уход × урожай → ценообразование. Слой почвы/климата/погоды
  — отдельная итерация (нужны источники).
