...
- Тесты: всё агро+земля+погода → **71 passed**.

## ⏳ В плане (не сделано)

### P1 — расширение (по мере поступления данных)
- **Урожай по сборам:** оценочная вьюха собирает землю×насаждение×уход; для урожая
  нужны карты со **сборами** (harvest: variety/volume_kg/acidity) — наполнят
  `v_agro_harvest_*` и достроят оценку «качество/количество».
- **Сетевой прогон погоды:** `accumulated_since_planting` ходит в Open-Meteo —
  нужен исходящий доступ (в среде закрыт); offline-ядро готово. Запись истории в
  таблицу `weather_*` (если нужна персистентность) — по запросу.
- **Почва/климат-нормы:** тип почвы, агротех-характеристики — нужны источники
  (NSPD/почвенные карты), отдельная итерация.
