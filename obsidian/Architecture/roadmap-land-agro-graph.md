...
- **A. Парсер техкарты** (`agro_techcard.py`) → `agro_parcel` + `agro_crop_cycle`
  ...
  - Объём: парсер Excel/таблицы → строки + golden-тест; на ingest событий —
    прогон через `validate_event_attrs` (D готов). Наполнит вьюхи E реальными данными.
...
Весь разблокированный parser-слой (земля/граф/агро-агрегаты) закрыт. Осталось:
1. **A** — как только появится образец техкарты (критический путь §6); наполнит
   вьюхи E. На ingest — `validate_event_attrs` (D).
