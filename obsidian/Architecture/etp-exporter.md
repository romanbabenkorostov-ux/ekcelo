...
| Auto-export hook | --export флаг во всех ETL CLI | done | этот PR |
...
Комбинируется с `--dry-run`: при dry-run экспорт пропускается с сообщением
`[skip-export]`. Реализация — `parser/exporters/etp/auto_export.py`.

Пример «всё в одном» для типового workflow:

```bash
# Применить YAML от экономиста + сразу обновить JSON для viewer
python -m parser.exporters.etp.etl_osv_cli \
...
# → viewer fetch при следующем заходе подхватит изменения
```

Файл коммитится в репо вручную (CI hook — отдельная задача).
