...
**Морфологические фильтры (pymorphy3):** `inflect_nom`, `inflect_gen`, `inflect_dat`, `inflect_acc`, `inflect_ins`, `inflect_loc`. Применяются в шаблоне как `{{ value | inflect_loc }}` для согласования падежей русских фраз («Российская Федерация» → «Российской Федерацией»). Реализация — `parser/exporters/etp/morphology.py`. Smart-skip: слова в родительном падеже внутри фразы не трогаются (например, «центральной части города» при `inflect_loc` остаётся корректным).
...
| PDF/DOCX-конверсия `lot_appendix` | `--appendix-format pdf\|docx`, LibreOffice fallback | done | #79 |
| Bulk-pipeline CLI | `etl_pipeline_cli` — сканирует inbox + `_applied/` rotation | done | #80 |
| Smoke-CLI | End-to-end проверка: import + init-db + cli + export-json | done | этот PR |
...
| ETL EXIF → БД | Автозаполнение из UserComment фото | план | — |
...
python -m parser.exporters.etp.etl_osv_cli \
    ...
    --db   path/to/ekcelo.sqlite \
    [--dry-run] [--export] [--export-out <dir>] [--export-project <slug>]
```

Контракт YAML: `obsidian/Architecture/etl-osv.md`.

С флагом `--export` сразу после применения ETL перегенерируется
JSON-экспорт для viewer (`parser/exports/etp/object_etp_profile.json`)
— см. секцию «Auto-export hook» ниже.
...
### CLI: NSPD-обогащение (Stage 5)

```bash
python -m parser.exporters.etp.nspd_enrich_cli \
    --db   path/to/ekcelo.sqlite \
    --nspd path/to/_data/nspd_cache/ \
    [--source nspd] [--confidence 0.8] [--dry-run] [--export]
```

Читает JSON-файлы из директории (формат NSPD v7+ `session_export_*.json`
или одиночные `<cad>.json`), gap-fill пишет в `object_etp_profile`:
- `wall_material` → `building_extra.building_type` (нормализован: «кирпич» → «кирпичное»).
- `year_built`/`year_used` → `building_extra.year_built`.
- `permitted_uses` → `legal_extra.use_type_permitted`.

**Никогда не перезаписывает существующие значения** — ручной ввод
экономиста (`osv`/`manual`) в приоритете. Новые записи создаются с
`source='nspd'`, `confidence=0.8`.

### CLI: EXIF-обогащение (Stage 6)

```bash
python -m parser.exporters.etp.etl_exif_cli \
    --db    path/to/ekcelo.sqlite \
    --photos path/to/Фотографии/ \
    [--source exif] [--confidence 0.7] [--dry-run] [--export]
```

Парсит EXIF UserComment v1.1 из JPG, группирует по `cad`, пишет сводку
«Комплексная фотофиксация: <категории>» в `extras.advantages[]` (gap-fill,
идемпотентно).

### Auto-export hook (`--export`) и auto-commit hook (`--commit`)

Все три ETL-CLI (`etl_osv_cli` / `nspd_enrich_cli` / `etl_exif_cli`)
поддерживают единый набор флагов:

| Флаг | Значение |
|---|---|
| `--export` | После успешного commit'а перегенерировать JSON-экспорт для viewer. |
| `--export-out <dir>` | Корневая директория экспорта (default: `parser/exports/etp/`). |
| `--export-project <slug>` | Project-фильтр (default: всё). |
| `--commit` | Дополнительно `git add` + `git commit` экспортированного JSON. |
| `--commit-author "Name <email>"` | Override автора коммита (default: `git config`). |

Комбинируется с `--dry-run`: при dry-run оба этапа пропускаются.

Поведение `--commit`:
- Если каталог не внутри git-репо → `[commit-skipped]` без падения.
- Если экспортированный файл не изменился → `[commit-noop]` (нет пустого commit'а).
- При успехе → `[committed] <sha> <path>`.
- Commit message: `chore(etp): auto-export <file> from <osv|nspd|exif>`.

Реализация — `parser/exporters/etp/auto_export.py`.

### CLI: smoke-тест end-to-end

```bash
python -m parser.exporters.etp.smoke_cli
# отладка: оставить tmp-директорию
python -m parser.exporters.etp.smoke_cli --work-dir ./smoke-run --keep
```

Гоняет в tmp-директории (без побочных эффектов на репо):

1. import-check — `importlib.import_module` для всех 20 модулей пакета.
   Ловит «missing-module» инциденты (см. `etp-local-sync.md`) до запуска CLI.
2. `init_db_cli --with-template` — SQLite + 3 объекта + профиль + лот.
3. db-rows — проверка `objects=3 / object_etp_profile=1 / lots=1 / lot_items=2`.
4. `cli` (Stage 3) — экспортирует `lot:pirushin:001` × `torgi.gov.ru` × `short,full`.
5. artifacts — существование и непустота `description.{short,full}.txt`,
   `lot_appendix.md`, `long_description.json`.
6. `export_json_cli` + json payload — ключи `$schema_version` / `object_etp_profile`
   / `lots` / `lot_items` присутствуют и непустые.

Exit code `0` — все 32 проверки прошли, `1` — провал хотя бы одной (детали в stderr).

Назначение: CI / release validation / профилактика «missing-module» инцидентов.
Реализация — `parser/exporters/etp/smoke_cli.py`. Тесты — `parser/tests/test_smoke_cli.py`.

Пример «всё в одном» — typical workflow экономиста:

```bash
# Применить YAML, обновить JSON для viewer, закоммитить — одна команда
python -m parser.exporters.etp.etl_osv_cli \
...
# → ETL UPSERT в БД
# → JSON-экспорт обновлён
# → git commit -m "chore(etp): auto-export object_etp_profile.json from osv"
# → остаётся git push (вручную, чтобы экономист видел дельту перед публикацией)
```

`--commit` не пушит и не сжимает — это пользовательский (или CI) шаг.
...
1. Экономист правит YAML  ──▶  parser/inbox/etp/<date>-<slug>.yml
                                      │
                                      ▼
2. parser-A: etl_osv_cli --export   ──▶  БД (UPSERT) + JSON-экспорт за один вызов
                                      │
                                      ├──▶ viewer fetch (admin/etp-profile/<cad>)
                                      ▼
3. parser-A: cli (Stage 3) ──▶  out/etp/<lot>/description.{short,full}.txt
                                                + lot_appendix.md
                                      │
                                      ▼
4. Оператор загружает на ЭТП.
```
...
- ✅ **Stage 4b: export JSON** — `parser/exports/etp/[<project>/]object_etp_profile.json`, viewer fetch'ит напрямую.
- ✅ **`building.building_type`** ← NSPD `wall_material`, нормализован («кирпич» → «кирпичное»). Stage 5.
- ✅ **`building.year_built`** ← NSPD `year_built` / `year_used`. Stage 5.
- ✅ **`legal.use_type_permitted`** ← `objects.permitted_use` (ЕГРН) с overlay от NSPD `permitted_uses`. Stage 5.

Открыто:
- **EXIF UserComment → БД.** Автозаполнение из фото объекта (план).
...
- **Auto-export hook:** после `etl_osv_cli` автоматически дёргать `export_json_cli` + коммит — viewer всегда актуален (план).
