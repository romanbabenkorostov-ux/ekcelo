...
| Этап | Что | Статус | PR |
|---|---|---|---|
| SPEC | `docs/etp_export/SPEC_etp_export.md` + матёрки 00..05 | done | #41 |
| ADR-001 | Решение по расширению БД | done · Accepted | #48 |
| 025 + 026 | Согласование с viewer-team (5 вопросов) | done · 5/5 ack | #50, #52 |
...
| **Stage 3** | CLI + PDF-приложение + integration test | в работе | — |
| ETL ОСВ → БД | Импорт ручных правок экономиста | план | — |
| EXIF → БД | Парсер UserComment для гэп-полей | план | — |
| viewer Phase 2 | Overlay лотов на карте | YAGNI | — |
| viewer Editor | `admin/etp-profile/<cad>` UI | план | — |
...
Документированы в SPEC §10 (12 пунктов). На 2026-05-28 закрыты:
- Структура БД (миграция 0001).
- Источник правды по полям — фикстура (production-источник: ETL из ОСВ + EXIF, план).
...
Открыто:
- **Компонентный адрес.** `location.region`/`/.../room` = None. `full_address` фолбэчит на `address_raw`. Нужен `address_parser.py` (Stage 3+).
- **`legal.encumbrances[].influence`.** Текстовое описание влияния обременения — None. Нужен `encumbrance_mapper.py` (Stage 3+).
- **`building.building_type` / `year_built` / `legal.use_type_permitted`.** Нужен NSPD-enrichment (Stage 4+).
