...
## Frontend — ekcelo-site

**Размещение:** монорепо subdir `ekcelo/ekcelo-site/` (выбор владельца, 2026-06-21).

### FE-0 — OAuth M2 browser code-flow
**Статус:** ✅ done (PR #120, см. `cycle-14-m2-browser-flow.md`). 15 тестов.

### FE-1 — scaffold + api-adapter + базовый UI
**Статус:** ✅ done (2026-06-21, см. `fe-1-ekcelo-site-scaffold.md`). Vite + TypeScript, структура core/adapters/ui по SPEC_frontend. ApiClient (C4) + каталог/объект/граф. 29 vitest, 0 typecheck/lint ошибок.

### FE-2 — интерактивный граф + kmz→ViewModel
**Статус:** ✅ done (2026-06-22, см. `fe-2-graph-kmz-adapter.md`). Интерактивный SVG-граф (радиальный layout, hover, клик-навигация) + `adapters/kmz.ts` (fflate ZIP + DOMParser KML → ViewModel). Офлайн drag-drop KMZ. 21 тест на реальном sample парсера. DoD выполнен: объект из api и kmz рисуется одним UI.

### FE-3 — карта + UI грантов
**Статус:** ✅ done (2026-06-22, см. `fe-3-map-grants.md`). Карта на Leaflet (geo.geometry из KMZ, динамический импорт + code-split) + UI грантов (список/выдача/отзыв через REST /grants, C6). 16 тестов (всего 66 vitest). **Frontend трек FE-0..FE-3 завершён** — ekcelo-site покрывает DoD SPEC_frontend.

---

## Cycle 16 — Rate limiting на auth-провалы

**Статус:** **✅ ЗАКРЫТ** (2026-06-21, см. `cycle-16-ratelimit.md`) — `RateLimiter` (in-memory, конфиг через env `EKCELO_RATELIMIT_*`); integration в BasicAuthMiddleware (key `basic:{ip}:{user}`) + OAuthMiddleware (key `bearer:{ip}`); 22 теста. **Закрывает auth-трек 12-13-14-15-16 целиком.** Redis-backend для multi-worker — отдельный sub-stage по необходимости. **Реализует:** hardening (не привязан к C-контракту).
