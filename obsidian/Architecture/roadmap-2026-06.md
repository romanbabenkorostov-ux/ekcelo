...
**Статус:** **M1+M2 ✅ done** (2026-06-15, см. `cycle-15-rbac.md`) — M1 (Principal/Grant/can/delegate/share + InMemoryGrantStore, 44 теста), M2 (SQLiteGrantStore в отдельной access.sqlite, 25 тестов; миграция `schema/migrations/access/0001_access_grants.sql`). M3 (FastAPI Depends + REST `POST/DELETE /grants`) — план. **Реализует:** `contracts/roles/ROLES_SPEC.md` (C6) целиком. **Триггер:** OAuth (cycle 14) приземлён ✅ ИЛИ появление assessor/client сценария.
