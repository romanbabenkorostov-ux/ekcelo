...
**Статус:** **M1 ✅ done** (2026-06-14, см. `cycle-15-rbac.md`) — Principal/Grant/can/delegate/share + InMemoryGrantStore, 44 теста. M2 (SQLite access_grants) и M3 (FastAPI Depends + REST `POST/DELETE /grants`) — план. **Реализует:** `contracts/roles/ROLES_SPEC.md` (C6) целиком. **Триггер:** OAuth (cycle 14) приземлён ✅ ИЛИ появление assessor/client сценария.
