...
**Статус:** **M1+M2+M3 ✅ done** (2026-06-15, см. `cycle-15-rbac.md`) — M1 (Principal/Grant/can/delegate/share + InMemoryGrantStore, 44 теста), M2 (SQLiteGrantStore в отдельной access.sqlite, 25 тестов; миграция `schema/migrations/access/0001_access_grants.sql`), M3 (FastAPI `require_action` dependency + REST `POST/DELETE /grants` + `GET /grants/me`, 19 тестов). M4 (enforcement wire-up в боевые роуты через `create_app(enforce_rbac=True)`) — план. **Реализует:** `contracts/roles/ROLES_SPEC.md` (C6) целиком. **Триггер:** OAuth (cycle 14) приземлён ✅ ИЛИ появление assessor/client сценария.
