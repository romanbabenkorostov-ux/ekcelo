...
**Статус:** Ратифицирован (PR #1, мерж 2026-05-18); 2.13.0 — `shared/correspondence-031`.
**Версия контракта:** 2.13.0 · **SemVer-политика:** см. §4.
...
| 2.13.0 | 2026-08-21 | MINOR: новый §11 «Профиль B — активы инвентаризации» — признание домена, ранее описанного только в `ekcelo-site/docs/CONTRACT_KMZ.md` (3.0.0+), как сосуществующего с Профилем A в одном `.kmz`; резерв `styleUrl`-префикса `inv_*`; уточнение §5/§6 (запрет HTML/`<img>`) как применимого только к Профилю A. Контрактная поверхность §2 не расширяется — Профиль B эмитируют не parser/viewer этого репозитория. Закрывает handoff, обещанный с 2.12.0/2.13.0 (`ekcelo-site` статус-блок). Аддитивно, byte-stable для Профиля A | `shared/correspondence-031`; `docs/CORRESPONDENCE/031` |

## 11. Профиль B — активы инвентаризации (домен вне парсера, сосуществует в одном .kmz)

Добавлено 2.13.0 (`docs/CORRESPONDENCE/031`). Контрактная поверхность §2
(`project.kmz` парсера) этим НЕ расширяется и не переопределяется — Профиль
B эмитируют не parser/viewer этого репозитория, а мобильное/десктопное
приложения инвентаризации и `VineInvent`/`iarobo-welcard`. Причина, по
которой этот файл вообще должен о нём знать: пользователь вправе объединить
кадастровую выгрузку (Профиль A) и инвентаризационную (Профиль B) в ОДНОМ
`.kmz`-контейнере для просмотра в `?mode=pro` — тогда оба профиля читаются
из одного `doc.kml` эвристиками по `styleUrl`-префиксу (§2.1 в
нормативном теле Профиля B).

**Нормативное тело Профиля B — `ekcelo-site/docs/CONTRACT_KMZ.md` §4–§7**
(версия 3.0.0+), по ссылке, не дублируется здесь. Краткая сводка того, что
касается ЭТОГО файла:

- **`styleUrl`-префикс `inv_*` зарезервирован за Профилем B** и не
  пересекается ни с одним из 9 префиксов §6 (`cad_zu_`, `cad_oks_`,
  `cad_room_`, `cad_str_`, `cad_ons_`, `cad_bu_`, `cad_eq_`, `cad_ben_`,
  `cad_exp_`) и с `photoPin_`. Парсер не эмитит и не обязан распознавать
  `inv_*` — просто гарантированно с ним не столкнётся.
- **Запрет HTML/`<img>` в `<description>` (§5/§6, «без HTML, без `<img>`»)
  действует только на Профиль A.** Профиль B пользуется HTML-`<table>` как
  разрешённым исключением вне контрактной поверхности §2 — это не изменение
  самого запрета (Профиль A остаётся byte-stable), а формальное признание
  границы его области действия, которая де-факто существовала и раньше
  (парсер никогда не читал/не писал Профиль B).
- Медиа Профиля B лежат в `files/` (не в `images/`/`docs/` §2) — отдельное
  пространство путей, коллизий с парсерными `images/<f>`/`docs/<f>` быть не
  может по построению (разные корневые папки в zip).
- Сайдкар Профиля B `ekcelo.json` — отдельно от `_data/documents.json` §5
  (разные пути, разные схемы, независимое присутствие).
- CI-инварианты §6 этого файла Профиля B не касаются и не проверяют его
