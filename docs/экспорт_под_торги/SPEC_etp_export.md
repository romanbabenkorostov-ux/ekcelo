...
Источник целевой модели описания, согласованный с экономистом, см. в `docs/экспорт_под_торги/` (файлы 00–05): структура `long_description`, абзацные шаблоны для ГИС Торги / Росэлторг / Сбербанк-АСТ и Jinja-движок генерации.
...
Каноническая JSON-схема и Jinja-плейсхолдеры с условиями включения — в `docs/экспорт_под_торги/` (файлы 03 и 05). Они переезжают в репо как есть.
...
parser/exporters/etp/
  ...
  templates/
    torgi_long_description.j2 # из docs/экспорт_под_торги/05 (как есть)
    macros/
      address.j2
      encumbrances.j2
      risks.j2
...
1. **Контракт JSON**: фикстура `tests/fixtures/etp/lot_sample.json` валидируется JSON-схемой из `docs/экспорт_под_торги/03` (раздел «JSON-схема плейсхолдеров»).
