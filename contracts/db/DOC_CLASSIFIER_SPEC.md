...
source: EGRUL          # питает атрибуты Subject и FOUNDER_OF/MANAGES, НЕ OWNS объекта (поправка)
anchor_keys: [inn, ogrn]
enriches:
  subjects: {inn, ogrn, name_current, subject_type: LEGAL_ENTITY}
  subject_kpp: [kpp...]                    # доп. КПП = обособленные подразделения
  subject_names: [{name_full, valid_from}] # история наименований (1 ИНН)
relations[subject]:
  FOUNDER_OF:  учредители → ЮЛ
  MANAGES:     директор/УК → ЮЛ
```
