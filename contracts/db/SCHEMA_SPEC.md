...
- subject-связи (corporate): `FOUNDER_OF, MANAGES, BRANCH_OF, CONTROLS, SUCCESSOR_OF, REORGANIZING_WITH`
  (`MANAGES` — руководитель «без доверенности», ФЛ/ЮЛ; `SUCCESSOR_OF` — правопреемство при
  реорганизации, `meta.reorg_type ∈ {merger,affiliation,division,spin_off,transformation}`)
